...
core::Real
PerResidueBfactorMetric::calculate( core::pose::Pose& mypose ) {
	core::Real b_factor = mypose.pdb_info().bfactor( Size const res, Size const atom_index );
	return b_factor;
}
void
		...
		basic::datacache::DataMap & datamap)
{
	...
	if (tag->hasOption("atom_type")){
		atom_type_ = tag->getOption<std::string>("atom_type",1);
